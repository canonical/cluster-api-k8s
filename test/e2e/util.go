...
func getK8sdClusterMembers(clientset *kubernetes.Clientset) ([]k8sdMember, error) {
	cmd := []string{"/snap/k8s/current/bin/k8s", "status", "--output-format", "yaml"}
	output, err := runCommandOnControlPlane(clientset, cmd)
	if err != nil {
...
func runCommandOnControlPlane(clientset *kubernetes.Clientset, cmd []string) (string, error) {
	podName := fmt.Sprintf("node-exec-%d", rand.Int31())

	klog.Infof("Creating pod %s...", podName)
	if err := createControlPlanePod(clientset, "default", podName, cmd); err != nil {
		return "", fmt.Errorf("failed to create pod: %w", err)
	}

	defer func() {
		fmt.Println("Deleting pod...")
		if err := deletePod(clientset, "default", podName); err != nil {
			klog.Errorf("Failed to delete pod: %v", err)
		}
	}()

	klog.Infof("Waiting for pod %s to complete...", podName)
	if err := waitForPodCompletion(clientset, "default", podName); err != nil {
		return "", fmt.Errorf("pod did not complete execution: %w", err)
	}

	logs, err := getPodLogs(clientset, "default", podName)
	if err != nil {
...
func createControlPlanePod(clientset *kubernetes.Clientset, namespace, podName string, cmd []string) error {
	pod := &v1.Pod{
...
func waitForPodCompletion(clientset *kubernetes.Clientset, namespace, podName string) error {
...
func getPodLogs(clientset *kubernetes.Clientset, namespace, podName string) (string, error) {
	...
	logStream, err := podLogRequest.Stream(context.TODO())
	if err != nil {
		return "", err
	}
