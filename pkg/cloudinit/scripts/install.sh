...
## - /capi/etc/snap-local-paths contains a space-separated list of local paths to snap files
##   to be installed, e.g.: /path/to/core20.snap /path/to/k8s.snap
...
elif [ -f "/capi/etc/snap-local-paths" ]; then
  snap_local_paths="$(cat /capi/etc/snap-local-paths)"
